<template>
    <div class="w-full mb-10">
        ...
        <!-- ================= BANNER ================= -->
        <div v-if="treatment" class="relative w-full mb-8 overflow-hidden">

            <img :src="treatmentImage" :alt="altText" class="w-full h-[180px] sm:h-[240px] md:h-[380px] object-cover" />
        ...
        <!-- ================= CONTENT ================= -->
        <div v-if="treatment" class="max-w-7xl mx-auto px-4">
            ...
            <div class="grid md:grid-cols-3 gap-8">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="text-gray-700 leading-relaxed text-[15px]" v-html="treatment.description"></div>
...
const fetchTreatment = async () => {
    ...
    try {
        ...
        const res = await fetch(
            "/api/method/someshwara_hospital.api.treatments.get_treatments"
        )
        ...
        if (data.message?.status === "success") {
            ...
            treatment.value = data.message.data.find(
...
const updateSEO = () => {
    ...
    const title =
        treatment.value.meta_title ||
        treatment.value.name1 ||
        "Someshwara Hospital"

    const description =
        treatment.value.meta_description ||
        "Someshwara Hospital Treatments"

    const keywords =
        treatment.value.meta_keyword ||
        "hospital, treatment, someshwara hospital"
...
/* ================= IMAGE ================= */
const treatmentImage = computed(() => {

    if (!treatment.value?.thumnail_image) return ""

    return treatment.value.thumnail_image.startsWith("http")
        ? treatment.value.thumnail_image
        : treatment.value.thumnail_image
})
...
const altText = computed(() =>
    ...
    treatment.value?.name1
        ? `Someshwara Hospital - ${treatment.value.name1}`
        : "Someshwara Hospital"
)
        ...
        },
        {
            validator: (rule, value, callback) => {
                ...
                if (!/^[6-9]\d{9}$/.test(digits)) {
                    ...
                    callback(
                        new Error("Enter valid 10 digit mobile number")
                    )
